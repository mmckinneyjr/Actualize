...
#uncomment ---- puts people.sort{|x, y| x["age"] <=> y["age"]}
...
#uncomment ---- puts people.sort{|x, y| [x[:age],x[:name]] <=> [y[:age],y[:name]]}
...
#uncomment ---- puts movies.sort{|x, y| x[:title] <=> y[:title]}.map{|x| x[:title]}
...
#uncomment ---- puts haystack[:hay][3][:hay][:hay][1][:hay][2]
...
def find_missing_letter(string)
  ...
  if string.empty? 
    ...
    (a.split(string[-1], 0)[1]).nil? ? nil : a.delete!(a.split(string[-1], 0)[1])
    (a.split(string[0], 0)[0]).nil? ? nil : a.delete!(a.split(string[0], 0)[0])
    ((a.split("") - string.split("")).join() == "") ? nil : (a.split("") - string.split("")).join()
  end
end
